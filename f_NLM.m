...
switch( n );
  case( 00 ) % Simulation ONLY
    %% HNLF ID: Simulation
    HNLF.Len   = 100;          % m, length of HNLF
    HNLF.ZDW   = 1547.00e-9;   % m, zero dispersion wavelength
  % HNLF.D1    = 0.017;        % ps/km/nm^2, 
    HNLF.D1    = 0.017e3;      %  s/    m^3, 
    HNLF.D2    = 0.000;        % ps/nm^3/km, slope of HNLF dispersion slope,       
  % HNLF.Gamma = 11.3 ;        % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.Gamma = 11.3e-3;      % 1/(W* m), Gamma, Nonlinear coefficient
    HNLF.alpha_dB = 0.0;                % dB/km, HNLF_Loss/(HNLF_Len/1e3);
    HNLF.alpha    = HNLF.alpha_dB/4343; % (-1/50)*log(10^-(Alpha_dB/10)); 
  case( 01 ) % 
    %% HNLF ID: 131233340001
    HNLF.Len   = 204;           % m, length of HNLF
    HNLF.ZDW   = 1548.00e-9;    % m, zero dispersion wavelength
    HNLF.D0_15 = 0.04;          % ps/nm/km, HNLF dispersion @ 1550nm
    HNLF.D1_15 = 0.017;         % ps/nm^2/km, HNLF dispersion slope @ 1550nm
    HNLF.D0    = 0.017e3*(Lambda_0 - HNLF.ZDW); % s/m^1, 
  % HNLF.D1    = 0.017;         % ps/m/nm^2, 
    HNLF.D1    = 0.017e3;       %  s/   m^2, 
  ...
  % HNLF.D2    =-0.003e-3;      % ps/nm^3/m, slope of HNLF dispersion slope, 
    ...
    HNLF.Loss  = 0.77;          % dB, total loss @1550
    HNLF.PMD   = 0.04;          % ps, total PMD C-band
    HNLF.Gamma = 11.3;          % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.alpha_dB = 0.900;              % dB/km, HNLF_Loss/(HNLF_Len/1e3);
    HNLF.alpha    = HNLF.alpha_dB/4343; % (-1/50)*log(10^-(Alpha_dB/10));
  case( 02 ) % 
    %% HNLF ID: 131233340002
    HNLF.Len   = 300;           % m, length of HNLF
    HNLF.ZDW   = 1548.00e-9;    % m, zero dispersion wavelength
    HNLF.D0    = 0.03;          % ps/km/nm  , HNLF dispersion
    HNLF.D1    = 0.017;         % ps/km/nm^2, HNLF dispersion slope
    HNLF.D2    = 0.002;         % ps/km/nm^3, slope of HNLF dispersion slope, 
                                % calculated using D0 = ax^2 + bx + c
                                % a = 0.001, b = 0.013, c = 0
    HNLF.Loss  = 0.75;          % dB, total loss @1550
    HNLF.PMD   = 0.04;          % ps, total PMD C-band
    HNLF.Gamma = 11.3;          % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.alpha_dB = 0.900;              % dB/km, HNLF_Loss/(HNLF_Len/1e3);
    HNLF.alpha    = HNLF.alpha_dB/4343; % (-1/50)*log(10^-(Alpha_dB/10));
  case( 03 ) % 
    ...
    HNLF.Len   = 507;           % m, length of HNLF
    HNLF.ZDW  = 1547.00e-9;    % m, zero dispersion wavelength
  % HNLF.D0    = 0.06;          % ps/km/nm  , HNLF dispersion
    HNLF.D0    = 0.04e-6;       % ps/    m^2, HNLF dispersion
  % HNLF.D1    = 0.017;         % ps/km/nm^2, HNLF dispersion slope
    HNLF.D1    = 0.017e3;       %  s/    m^3, HNLF dispersion slope
  % HNLF.D2    = 0.000;         % ps/km/nm^3, slope of HNLF dispersion slope
    HNLF.D2    = 0.000e12;      % ps/    m^4, slope of HNLF dispersion slope
    HNLF.Loss  = 1.05;          % dB, total loss @1550
    HNLF.PMD   = 0.04;          % ps, total PMD C-band
  % HNLF.Gamma = 11.3;          % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.Gamma = 11.3e-3;       % 1/(W* m), Gamma, Nonlinear coefficient     
    HNLF.alpha_dB = 0.900;              % dB/km, HNLF_Loss/(HNLF_Len/1e3);
  ...
  case( 05 ) % 
    ...
    HNLF.Len   = 255;           % m, length of HNLF
    HNLF.ZDW   = 1566.00e-9;    % m, zero dispersion wavelength
    HNLF.D0    = -1.30;         % ps/nm/km, HNLF dispersion
    HNLF.D1    = 0.083;         % ps/nm^2/km, HNLF dispersion slope
    HNLF.Loss  = 0.64;          % dB, total loss @1550
    HNLF.PMD   = 0.05;          % ps, total PMD C-band @1550nm
    HNLF.Gamma = 8.7;           % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.EffAr = 16.1;          % um^2, effective area @1550nm
    HNLF.alpha_dB = 0.960;      % dB/km, HNLF_Loss/(HNLF_Len/1e3);
  case( 06 ) % 
    %% HNLF ID: 14251113090002
    HNLF.Len   = 256;           % m, length of HNLF
    HNLF.ZDW   = 1566.00e-9;    % m, zero dispersion wavelength
    HNLF.D0    = -1.30;         % ps/nm/km, HNLF dispersion
    HNLF.D1    = 0.083;         % ps/nm^2/km, HNLF dispersion slope
    HNLF.Loss  = 0.61;          % dB, total loss @1550
    HNLF.PMD   = 0.05;          % ps, total PMD C-band @1550nm
    HNLF.Gamma = 8.7;           % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.EffAr = 16.1;          % um^2, effective area @1550nm
    HNLF.alpha_dB   = 0.960;    % dB/km, HNLF_Loss/(HNLF_Len/1e3);
  case( 07 ) % 
    ...
    HNLF.Len   = 255+256;       % m, length of HNLF
    HNLF.ZDW   = 1566.00e-9;    % m, zero dispersion wavelength
  % HNLF.D0    =-1.30;          % ps/km/nm^1, HNLF dispersion @1550nm
    HNLF.D0    =-1.30e-6;       %  s/    m^2, HNLF dispersion @1550nm
  % HNLF.D1    = 0.083;         % ps/km/nm^2, 
    HNLF.D1    = 0.083e3;       %  s/    m^3, HNLF dispersion slope
  % HNLF.D2    = 0.000;         % ps/km/nm^3, slope of HNLF dispersion slope 
    HNLF.D2    = 0.000e12;      %  s/    m^4, slope of HNLF dispersion slope
    HNLF.Loss  = 0.64+0.61;     % dB, total loss @1550
    HNLF.PMD   = 0.05;          % ps, total PMD C-band 
  % HNLF.Gamma = 8.7 ;          % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.Gamma = 8.7e-3;        % 1/(W* m), Gamma, Nonlinear coefficient     
    HNLF.alpha_dB = 0.960;              % dB/km, HNLF.Loss/(HNLF.Len/1e3);
    HNLF.alpha    = HNLF.alpha_dB/4343; % (-1/50)*log(10^-(Alpha_dB/10));       
  case( 09 ) % 
    %% HNLF ID: XXXXXXXXXXXXXX
    HNLF.ZDW   = 1565.00e-9;    % m, zero dispersion wavelength
  % HNLF.D0    = 2.0e-6;        % ps/(nm  *m), dispersion
    HNLF.D0    = DD*(Lambda_0 - HNLF.ZDW); % ps/(nm*km), dispersion calculated by slope
  % HNLF.D1    = 0.027;         % ps/(nm^2*m), dispersion slope 1st derivative
    HNLF.D1    = 0.027e3;       %  s/      m^3, dispersion slope 1st derivative
    HNLF.D2    = 0.000e0;       % ps/(nm^3*m), slope of dispersion slope, second derivative 
  case( 10 ) % 
    %% HNLF ID: XXXXXXXXXXXXXX
    HNLF.Len   = 100;
    HNLF.ZDW   = 1550.00e-9; % m, zero dispersion wavelength
  % HNLF.D0    = 0.017e3*(Lambda_0 - HNLF.ZDW); % s/m^1, 
  ...
  % HNLF.D2    = 0.005;         % ps/km/nm^3, slope of HNLF dispersion slope
    HNLF.D2    = 0.000e12;      % ps/    m^4, slope of HNLF dispersion slope
    HNLF.Loss  = 0.10;          % dB, total loss @1550
    HNLF.PMD   = 0.05;          % ps, total PMD C-band 
  % HNLF.Gamma = 10.0 ;         % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.Gamma = 10.0e-3;       % 1/(W* m), Gamma, Nonlinear coefficient     
    HNLF.alpha_dB = 0.000;              % dB/km, HNLF.Loss/(HNLF.Len/1e3);
    HNLF.alpha    = HNLF.alpha_dB/4343; % (-1/50)*log(10^-(Alpha_dB/10));    
  case( 11 ) % 
    %% 2014.ECOC Maxime Baillot, 6-wave
    HNLF.Len   = 10.40e3;       % m, length of HNLF
    HNLF.ZDW   = 1548.00e-9;    % m, zero dispersion wavelength
  % HNLF.D1    = 0.077;         % ps/(km*nm^2), HNLF dispersion slope
    HNLF.D1    = 0.077e3;       %  s/      m^3, HNLF dispersion slope in International Unit
    HNLF.D2    = 0.000;         % ps/(km*nm^3), slope of HNLF dispersion slope
    HNLF.Gamma = 3.30e-3;       % 1/(W*m), Gamma, Nonlinear coefficient
    HNLF.alpha_dB = 0.200;              % 0.200 dB/km, HNLF_Loss/(HNLF_Len/1e3);      
    HNLF.alpha    = HNLF.alpha_dB/4343; %  losses in m-1, 10^(-Alpha_dB/(10*1e3));
  case( 12 ) % 
    %% 2009.CLEO Mingyi Gao, 9-wave 
    HNLF.Len   = 1000;          % m, length of HNLF
    HNLF.ZDW   = 1565.00e-9;    % m, zero dispersion wavelength
    HNLF.ZDF   = c/HNLF.ZDW;    % Hz, zero dispersion frequency
  % HNLF.D0    = 0.000;         % ps/km/nm^1, HNLF dispersion @1550nm
    HNLF.D0    = 0.000e-6;      %  s/    m^2, HNLF dispersion @1550nm
  % HNLF.D1    = 0.027;         % ps/km/nm^2, HNLF dispersion slope
    HNLF.D1    = 0.027e3;       %  s/    m^3, HNLF dispersion slope
  % HNLF.D2    = 0.000;         % ps/km/nm^3, slope of HNLF dispersion slope
    HNLF.D2    = 0.000e12;      %  s/    m^4, slope of HNLF dispersion slope
  % HNLF.Gamma = 15.0 ;         % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.Gamma = 15.0e-3;       % 1/(W* m), Gamma, Nonlinear coefficient
    HNLF.alpha_dB = 0.001;              % dB/km, HNLF_Loss/(HNLF_Len/1e3);
    HNLF.alpha    = HNLF.alpha_dB/4343; % (-1/50)*log(10^-(Alpha_dB/10));
  case( 13 ) % 
    %% 2012.OFC Mingyi Gao, 7-wave 
    HNLF.Len   = 600;           % m, length of HNLF
    HNLF.ZDW   = 1541.67e-9;    % m, zero dispersion wavelength
    HNLF.ZDF   = c/HNLF.ZDW;    % Hz, zero dispersion frequency
  % HNLF.D0    = 0.494;         % ps/km/nm^1, HNLF dispersion @1550nm
    HNLF.D0    = 0.494e-6;      % ps/    m^2, HNLF dispersion @1550nm
  % HNLF.D1    = 0.025;         % ps/km/nm^2, HNLF dispersion slope
    HNLF.D1    = 0.025e3;       %  s/    m^3, HNLF dispersion slope
    HNLF.D2    = 0.000;         % ps/km/nm^3, slope of HNLF dispersion slope, 
  % HNLF.Gamma = 15.0 ;         % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.Gamma = 10.0e-3;       % 1/(W* m), Gamma, Nonlinear coefficient
    HNLF.alpha_dB = 0.670;              % dB/km, HNLF_Loss/(HNLF_Len/1e3);
    HNLF.alpha    = HNLF.alpha_dB/4343; % (-1/50)*log(10^-(Alpha_dB/10));
  case( 14 ) % 
  ...
  % HNLF.D0    = 0.482;         % ps/km/nm^1,  
    HNLF.D0    = 0.482e-6;      %  s/    m^2,  
  % HNLF.D0    = 0;             %  s/    m^2,  
  % HNLF.D1    = 0.025;         % ps/km/nm^2, 
    HNLF.D1    = 0.025e3;       %  s/    m^3, 
    HNLF.D2    = 0.000;         % ps/km/nm^3, slope of HNLF dispersion slope, 
  % HNLF.Gamma = 10.0 ;         % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.Gamma = 10.0e-3;       % 1/(W* m), Gamma, Nonlinear coefficient
    HNLF.alpha_dB = 0.670;              % dB/km, HNLF_Loss/(HNLF_Len/1e3);
  ...
  % HNLF.Len   = 1.5e-3;        % m, length of HNLF
    HNLF.Len   = 1.0e-3;        % m, length of HNLF
    HNLF.ZDW   = 1552.30e-9;    % m, zero dispersion wavelength
    HNLF.ZDF   = c/HNLF.ZDW;    % Hz, zero dispersion frequency
  % HNLF.D0    = 0.04;          % ps/nm/km^1, HNLF dispersion
    HNLF.D0    = 0.000e-6;      % ps/    m^2, HNLF dispersion
  % HNLF.D1    = 0.017;         % ps/km/nm^2, 
    HNLF.D1    = 0.000e3;       %  s/    m^3, HNLF dispersion slope
  % HNLF.D2    = 0.005;         % ps/km/nm^3, slope of HNLF dispersion slope
    HNLF.D2    = 0.000e12;      % ps/    m^4, slope of HNLF dispersion slope
  % HNLF.Loss  = 0.77+0.75+1.05;% dB, total loss @1550
  % HNLF.PMD   = 0.04;          % ps, total PMD C-band
  % HNLF.Gamma = 2000e3;        % 1/(W*km), Gamma, Nonlinear coefficient
    HNLF.Gamma = 2000.0;        % 1/(W* m), Gamma, Nonlinear coefficient     
    HNLF.alpha_dB = 30.0e5;          % dB/km, HNLF.Loss/(HNLF.Len/1e3);
    HNLF.alpha_dB = 00.0e5;          % dB/km, HNLF.Loss/(HNLF.Len/1e3);
    HNLF.alpha    = HNLF.alpha_dB/4343; % (-1/50)*log(10^-(Alpha_dB/10));
